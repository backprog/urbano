...
class Urbano {
    ...
    async.parallel([
      ...
      if (!err) {
        async.series([
          ...
          this.preServerLoad.bind(this),
          this.loadServer.bind(this),
          this.postServerLoad.bind(this)
        ], (err, results) => {
          if (err) {
  ...
  preServerLoad (callback) {
    if (this._config.configs.hasOwnProperty('server')) {
      ...
      if (conf.hasOwnProperty('preload')) {
        let fns = []
        let i = 0
        const l = conf.preload.length
        const Bundles = require(path.resolve(__dirname, 'bundles'))

        for (; i < l; i++) {
          const parts = conf.preload[i].split('.')
          const srvName = parts[0]

          if (this.services.has(srvName)) {
            const bundle = new Bundles(this)

            bundle.getService(srvName, (srv) => {
              fns.push(execPreLoad.bind(undefined, srv[parts[1]].bind(srv)))
            })
          } else {
            return callback(`Cannot find ${srvName} in server preloading`)
          }
        }
  ...
  onServerLoad (callback) {
    if (this._config.configs.hasOwnProperty('server')) {
      ...
      if (conf.hasOwnProperty('onload')) {
        let fns = []
        let i = 0
        const l = conf.onload.length
        const Bundles = require(path.resolve(__dirname, 'bundles'))

        for (; i < l; i++) {
          const parts = conf.onload[i].split('.')
          const srvName = parts[0]
          ...
          if (this.services.has(srvName)) {
            ...
            bundle.getService(srvName, (srv) => {
              fns.push(execOnLoad.bind(undefined, srv[parts[1]].bind(srv)))
            })
          } else {
            return callback(`Cannot find ${srvName} in server onloading`)
          }
        }
  ...
  loadServer (callback) {
    ...
    const start = (serverType, options, port) => {
      ...
      this._server.listen(port, (err) => {
        if (!err) {
          this.onServerLoad((err) => {
            if (!err) {
              this._winston.info(`Server ${serverType} listening on port ${port}`)
              return callback(null)
            } else {
              return callback(err)
            }
  ...
  postServerLoad (callback) {
    if (this._config.configs.hasOwnProperty('server')) {
      const conf = this._config.configs.server
      const execPostLoad = (fn, cb) => {
        try {
          fn(cb)
        } catch (e) {
          cb(e)
      ...
      if (conf.hasOwnProperty('postload')) {
        let fns = []
        let i = 0
        const l = conf.postload.length
        const Bundles = require(path.resolve(__dirname, 'bundles'))

        for (; i < l; i++) {
          const parts = conf.postload[i].split('.')
          const srvName = parts[0]
          ...
          if (this.services.has(srvName)) {
            ...
            bundle.getService(srvName, (srv) => {
              fns.push(execPostLoad.bind(undefined, srv[parts[1]].bind(srv)))
            })
          } else {
            return callback(`Cannot find ${srvName} in server postloading`)
          }
        }
