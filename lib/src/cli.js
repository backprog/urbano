...
class CLI {
  ...
  submitPlugin (callback = null) {
    const credentials  = fs.readFileSync(this.credential_path)
                        ...
                        request.post({
                          ...
                          if (!err) {
                            switch (httpResponse.statusCode) {
                              ...
                              case 400: console.log('Invalid submission. You did not provided a valid plugin. Most of the time it\'s due to an invalid plugin.json file'); break
  ...
  logIn (callback = null) {
    ...
    const onResponse = (err, httpResponse, body) => {
      if (!err && httpResponse.statusCode === 200) {
        ...
        if (callback) {
          callback(`Cannot find user ${input.email}`)
        }
      } else {
        if (callback) {
          callback('Repository unreachable')
