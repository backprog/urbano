...
class CLI {
  constructor () {
    ...
    this.def = [
      ...
      { name: 'autoLoadPlugins', alias: 'a' },
      { name: 'uninstall', alias: 'd', type: String },
  ...
  command () {
    ...
    if (this.options.hasOwnProperty('autoLoadPlugins')) {
      commands.push(this.autoLoadPlugins.bind(this))
    }
    if (this.options.hasOwnProperty('install')) {
      commands.push(this.installPlugins.bind(this, null))
    }
    if (this.options.hasOwnProperty('uninstall')) {
      commands.push(this.uninstallPlugins.bind(this))
    }
  ...
  autoLoadPlugins (callback = null) {
    this._findAppRoot((appRoot) => {
      if (appRoot) {
        const plugins = path.resolve(appRoot, 'plugins.json')
        let fns = []

        fs.stat(plugins, (err) => {
          if (!err) {
            try {
              const json = require(plugins)

              for (const plugin in json) {
                if (json.hasOwnProperty(plugin)) {
                  fns.push(this.installPlugins.bind(this, `${plugin}${json[plugin]}`))
                }
              }
            ...
            if (callback) {
              return callback('Cannot find a plugins.json file in your app\'s root directory')
            }
          }
        })
      } else {
  ...
  installPlugins (target = null, callback = null) {
    if (this.options.install || target) {
      this._findAppRoot((appRoot) => {
        if (appRoot) {
          let save = false
          let install = (this.options.install) ? this.options.install : target

          if (/:save/.test(install)) {
            install = install.replace(':save', '').trim()
            save = true
          }

          const plugin = this._getPluginInfo(install)
          ...
          if (plugin.name) {
            ...
            request.get(`${this.repository_url}/install/${install}`, (err, httpResponse, body) => {
              if (!err) {
                    ...
                    request.get(`${this.repository_url}/assets/repository/${filename}`)
                        ...
                        .on('response', (response) => {
                          if (response.statusCode !== 200) {
                            ...
                            return callback(`${install} not found`)
                          }
                        })
                        .on('complete', () => {
                          try {
                          ...
                          try {
                            ...
                            try {
                              ...
                              console.log(`${install} was installed but an error occurred while deleting the source in ${destination}`)
                            }

                            if (save) {
                              let plugins = {}
                              const jsonPlugins = path.resolve(appRoot, 'plugins.json')

                              fs.stat(jsonPlugins, (err) => {
                                if (!err) {
                                  plugins = require(jsonPlugins)
                                } else {
                                  if (err.code !== 'ENOENT') {
                                    return callback('An error occurred while loading plugins.json file. Couldn\'t save new plugin')
                                  }
                                }

                                plugins[plugin.name] = (plugin.scope) ? `${plugin.scope}${plugin.version}` : ''

                                try {
                                  jsonfile.writeFileSync(jsonPlugins, plugins, { spaces: 2 })
                                } catch (e) {
                                  console.log('Couldn\'t save new plugin to plugins.json file')
                                }
                              })
                            }
  ...
  uninstallPlugins (callback = null) {
    this._findAppRoot((appRoot) => {
      if (appRoot) {
        let save = false
        let uninstall = this.options.uninstall

        if (/:save/.test(uninstall)) {
          uninstall = uninstall.replace(':save', '').trim()
          save = true
        }
        const plugin = path.resolve(appRoot, 'src', 'plugins', uninstall)

        fs.stat(plugin, (err, stat) => {
          if (!err && stat.isDirectory()) {
            try {
              fs.removeSync(plugin)

              if (save) {
                try {
                  const jsonPlugins = path.resolve(appRoot, 'plugins.json')
                  let plugins = require(jsonPlugins)

                  if (plugins.hasOwnProperty(uninstall)) {
              ...
              if (callback) {
                return callback(`${uninstall} uninstalled`)
              }
            } catch (e) {
              if (callback) {
                return callback(e)
            ...
            if (callback) {
              return callback(`Could not find ${uninstall} plugin directory`)
            }
          }
        })
      } else {
